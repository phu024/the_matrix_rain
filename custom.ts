...
namespace Bits {
    ...
    //% weight=50 blockId="id_bwop" block="%op1 | %op | %op2"
    export function fn_bwop(op1: number, op: bwop, op2: number): number {
        if (op == bwop.and)
            return op1 & op2
        else if (op == bwop.or)
            return op1 | op2
    ...
    //% weight=40 blockId="id_not" block="NOT | %op1"
    export function fn_not(op1: number): number {
        return op1 ^ 0xffff
    }
}
//% weight=140 color=#0fbc11 icon="\uf061"
namespace SR_74HC595 {
    //% weight=100 blockId=id_enableoe block="enable output %oe"
    export function fn_enableOE(oe: DigitalPin): void {
        pins.digitalWritePin(oe, 0);
    }
    //% weight=90 blockId=id_disableoe block="disable output %oe"
    export function fn_disableOE(oe: DigitalPin): void {
        pins.digitalWritePin(oe, 1);
    }
    //% weight=80 blockId=id_sendbyte block="send byte %byte | SER %ser |  SRCLK %srclk | RCLK %rclk"
    export function fn_sendByte(byte: number, ser: DigitalPin, srclk: DigitalPin, rclk: DigitalPin): void {
        for (let i = 0; i < 8; i++) {
            pins.digitalWritePin(ser, byte & 1);
            pins.digitalWritePin(srclk, 0);
            pins.digitalWritePin(srclk, 1);
            byte >>= 1;
        }
        pins.digitalWritePin(rclk, 0);
        pins.digitalWritePin(rclk, 1);
        pins.digitalWritePin(ser, 0);
        pins.digitalWritePin(srclk, 0);
        pins.digitalWritePin(rclk, 0);
    }
}
